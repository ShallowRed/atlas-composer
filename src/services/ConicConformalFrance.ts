...
/* eslint-disable */
/**
 * This is the pasted version of d3-composite-projections's conicConformalFrance,
 * We should not touch the file by any means.
 */
...
export function fitExtent(projection, extent, object) {
  ...
  let b = boundsStream.result()
  let k = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1]))
  let x = +extent[0][0] + (w - k * (b[1][0] + b[0][0])) / 2
  let y = +extent[0][1] + (h - k * (b[1][1] + b[0][1])) / 2
...
export default function () {
  let cache
  let cacheStream
  let europe = conicConformal().rotate([-3, -46.2]).parallels([0, 60])
  let europePoint
  let guyane = mercator().center([-53.2, 3.9])
  let guyanePoint
  let martinique = mercator().center([-61.03, 14.67])
  let martiniquePoint
  let guadeloupe = mercator().center([-61.46, 16.14])
  let guadeloupePoint
  let saintBarthelemy = mercator().center([-62.85, 17.92])
  let saintBarthelemyPoint
  let stPierreMiquelon = mercator().center([-56.23, 46.93])
  let stPierreMiquelonPoint
  let mayotte = mercator().center([45.16, -12.8])
  let mayottePoint
  let reunion = mercator().center([55.52, -21.13])
  let reunionPoint
  let nouvelleCaledonie = mercator().center([165.8, -21.07])
  let nouvelleCaledoniePoint
  let wallisFutuna = mercator().center([-178.1, -14.3])
  let wallisFutunaPoint
  let polynesie = mercator().center([-150.55, -17.11])
  let polynesiePoint
  let polynesie2 = mercator().center([-150.55, -17.11])
  let polynesie2Point
  let point
  let pointStream = {
    point(x, y) {
  ...
  conicConformalFrance.invert = function (coordinates) {
    let k = europe.scale()
    let t = europe.translate()
    let x = (coordinates[0] - t[0]) / k
    let y = (coordinates[1] - t[1]) / k
